...
class GeminiWebRunnerBase:
    ...
    def login(self) -> None:
        with open_persistent_page(self.config) as (_context, page):
            ...
            print(
                "Sign into Google in the opened browser window. This command will finish automatically when Gemini is authenticated."
            )
            print(
                "If Google asks for 2FA or confirmation on another device, complete it there and come back here."
            )
            ...
            while time.time() < deadline:
                ...
                if is_logged_in(page):
                    print(
                        f"Gemini login completed. Persistent profile saved at: {self.config.profile_dir}"
                    )
                    return
                try:
    ...
    def _submit_prompt(self, page, prompt: str) -> None:
        ...
        if self._attempt_prompt_submission(page, candidates, prompt):
            return
    ...
    def _attempt_prompt_submission(self, page, candidates: list, prompt: str) -> bool:
    ...
    def _wait_for_generated_images(
        self, page, timeout_seconds: int, existing_images: set[str] | None = None
    ) -> list[dict]:
        deadline = time.time() + timeout_seconds
        ...
        while time.time() < deadline:
            ...
            new_images = [
                img
                for img in images
                if self._image_identity(img) not in existing_images
            ]
            if new_images:
                return new_images
            page.wait_for_timeout(4000)
        raise GenerationTimeoutError(
            f"Gemini image generation did not finish within {timeout_seconds} seconds"
        )
    ...
    def _wait_for_text_response(self, page, timeout_seconds: int) -> str:
        ...
        while time.time() < deadline:
            ...
            if answer and (
                saw_generating
                or "Ответ Gemini" in body_text
                or "Gemini response" in body_text
            ):
                return answer
            page.wait_for_timeout(2500)
        raise GenerationTimeoutError(
            f"Gemini chat response did not finish within {timeout_seconds} seconds"
        )
    ...
    def _collect_candidate_images(
        self, page, latest_response_only: bool = False
    ) -> list[dict]:
        try:
    ...
    def _extract_answer_text(self, body_text: str) -> str | None:
        ...
        for marker in markers:
            ...
            answer = re.sub(r"\n{3,}", "\n\n", answer)
            answer = re.sub(
                r"(?:\n|\s)*(Инструменты|Быстрая|Pro)\s*$", "", answer
            ).strip()
            if answer:
                return answer
        return None
    ...
    def _save_images(
        ...
        for idx, image in enumerate(images, start=1):
            if prefer_screenshot:
                screenshot_path = self._screenshot_image_element(
                    page, image, output_dir, idx
                )
                if screenshot_path:
    ...
    def _download_generated_images(self, page, output_dir: Path) -> list[str]:
        saved: list[str] = []
        try:
            buttons = page.locator("model-response").last.get_by_role(
                "button",
                name=re.compile(
                    "Скачать изображение|Скачать в полном размере|Download", re.I
                ),
            )
            count = buttons.count()
        except Exception:
        ...
        for idx in range(count):
            try:
                ...
                download = dl.value
                suggested = (
                    download.suggested_filename or f"generated-image-{idx + 1}.png"
                )
    ...
    def _screenshot_image_element(
        self, page, image: dict, output_dir: Path, idx: int
    ) -> str | None:
        try:
            if image.get("latest_response_only"):
                locator = (
                    ...
                    .last.locator(
                        "single-image img.image, generated-image img.image, img"
                    )
                    .nth(int(image.get("index", idx - 1)))
                )
            else:
    ...
    def _upload_files(self, page, files: list[str]) -> None:
        ...
        try:
            open_menu = page.get_by_role(
                "button",
                name=re.compile(
                    "Открыть меню загрузки файлов|Open upload file menu", re.I
                ),
            )
                ...
                for menu_label in [
                    ...
                    try:
                        ...
                        with page.expect_file_chooser(timeout=15000) as fc:
                            page.get_by_role("menuitem", name=menu_label).first.click(
                                force=True
                            )
                        chooser = fc.value
    ...
    def _wait_until_image_download_ready(self, page, timeout_seconds: int) -> None:
        ...
        while time.time() < deadline:
            ...
            page.wait_for_timeout(2000)
        raise GenerationTimeoutError(
            f"Gemini image download controls did not become ready within {timeout_seconds} seconds"
        )

    def _chat_missing(self, page) -> bool:
        body = self._read_body_text(page)
        return (
            "Не удалось загрузить этот чат" in body
            or "This chat couldn't be loaded" in body
        )
    ...
    def _detect_google_error(self, body_text: str) -> GeminiWebError | None:
        ...
        if re.search(
            r"(something went wrong|что-то пошло не так).{0,40}\(13\)",
            body_text,
            flags=re.IGNORECASE | re.DOTALL,
        ):
            return GoogleRuntimeError13(
                "Google Gemini returned Something went wrong (13)"
            )
