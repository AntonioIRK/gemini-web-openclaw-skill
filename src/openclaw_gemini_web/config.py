...
class GeminiWebConfig:
    ...
    def from_env(cls) -> "GeminiWebConfig":
        workspace_root = Path(os.environ.get("GEMINI_WEB_ROOT", os.environ.get("GEMINI_STORYBOOK_ROOT", Path.cwd()))).resolve()
        ...
        profile_dir = Path(
            os.environ.get(
                ...
                os.environ.get(
                    ...
                    legacy_profile_dir if legacy_profile_dir.exists() else default_profile_dir,
                ),
            )
        ).resolve()
